...
function forEachWaterIsSelected(water){
  ...
  if(geolib.getDistance(water.center, store.selectedPosition) <= x){
...
function forEachRoadIsSelected(road){
  ...
  if(geolib.getDistance(road.center, store.selectedPosition) <= x){
...
function forEachTourismIsSelected(tourism){
  ...
  if(geolib.getDistance(tourism.center, store.selectedPosition) <= x){
...
function forEachFiberSetCost(fiber){
  ...
  const newLoc = { lat: 40.0150, lon: -105.2705 }

  fiber.newDistance = geolib.getDistance(newLoc, store.selectedPosition);
  fiber.newCost = 5 * fiber.newDistance + (100000 * waterCount) + (100000 * roadCount);
