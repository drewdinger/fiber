...
//   $.ajax('/data/export.geojson').done(function(json) {
//     store.geometries = json
//     console.log(store.geometries);
//     let water = {}
//     store.water = _.map(json, (d) => {
...
export function setSelectedPosition(latlng) {
  ...
  _.forEach(store.water, forEachWaterIsSelected)
...
function forEachFiberSetCost(fiber){
  ...
  //$5 per meter
  fiber.cost = 5 * fiber.distance + (10000000/store.selectedPosition.waterDist);
}
