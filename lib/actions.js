...
function forEachFiberSetCost(fiber){
  ...
  console.log(fiber.distance);
  
  //$5 per meter
  fiber.cost = 5 * fiber.distance + (100/store.selectedPosition.waterDist) + (100/store.selectedPosition.roadDist) + (100/store.selectedPosition.tourismDist);
  }
  else 
  {
    fiber.cost = 0
